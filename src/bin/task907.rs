...
impl Problem {
    fn input() -> Self {
        ...
        let an: Vec<usize> = input.next_as_iter().collect();
        let q = input.next();
        let mut vn: Vec<usize> = Vec::with_capacity(q);
        for _ in 0..q {
            vn.push(input.next());
        }
    ...
    fn solve(&self) -> Vec<usize> {
        ...
        parent_of[1..].copy_from_slice(an);
...
fn rec(childs_of: &Vec<Vec<usize>>, route: &mut Vec<usize>, i: usize, v: usize) {
    if i != v {
        route.push(v);
    }
    for v in childs_of[v].iter() {
        rec(childs_of, route, i, *v);
    }
}
