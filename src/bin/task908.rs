...
impl Problem {
    fn input() -> Self {
        ...
        for _ in 1..n {
            let t = input.next_tuple();
            an.push(t);
        }
        let q = input.next();
        let mut vn = Vec::with_capacity(q);
        for _ in 0..q {
            let x = input.next();
            vn.push(x);
        }
    ...
    fn solve(&self) -> Vec<String> {
        let mut adj: Vec<Vec<usize>> = vec![Vec::with_capacity(self.n); self.n];
        for (a, b) in &self.an {
            adj[*a].push(*b);
            adj[*b].push(*a);
        }

        let mut children: Vec<Vec<usize>> = vec![Vec::with_capacity(self.n); self.n];
        let mut parent: Vec<usize> = vec![0; self.n];
        rec(0, 0, &adj, &mut children, &mut parent);

        for cs in children.iter_mut() {
            cs.sort_unstable();
        }

        let mut answers: Vec<String> = Vec::with_capacity(self.q);
        for v in &self.vn {
            let l = format_slice(&children[parent[*v]]);
            answers.push(l);
        }
