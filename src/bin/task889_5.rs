...
fn main() {
    ...
    let input = fast_input::FastInput::new();

    let _x: usize = input.next();
    let _q: usize = input.next();

    let mut scheduled_completion_times: VecDeque<usize> = VecDeque::new();
    let mut completed_task_counter: usize = 0;
    for _ in 0.._q {
        let (op_id, current_time): (usize, usize) = input.next_tuple();
        match op_id {
            0 => {
...
// Following code is from https://crates.io/crates/fast_input
mod fast_input {
    use std::cell::Cell;
    use std::fmt::Display;
    use std::io::prelude::*;
